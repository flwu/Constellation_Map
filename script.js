...
function displayGSMetadata(event) {
    ...
    if (selectedPts.length !== 0) {
        ...
        if (selectedPts.length === 1) { // only 1 gene set selected
            // Hide intersect/union form
            d3.select(".miniForm").classed("hidden", true);
            ...
            // Populate panel with metadata
            // 1. Gene Set Name(s)
            if (selectedPts.length === 2 && selectedEdge.length === 1) {
                d3.select("#liveTextOne")
                    .text("1 edge selected (2 gene sets). Jaccard = " + round(selectedEdge[0].Jaccard, 3)); // live text
            } else {
                d3.select("#liveTextOne")
                    .text(selectedPts.length + " gene sets selected."); // live text
            } // End if (selectedPts.length === 2)
